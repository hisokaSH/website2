...
const DISCORD_STATUS_MESSAGES = {
  ...
  already_linked:   { type: 'error',   text: 'That Discord account is already linked to another user.' },
  invalid_state:    { type: 'error',   text: 'Discord link session expired. Please try again.' },
  missing_params:   { type: 'error',   text: 'Discord authorization was cancelled.' },
  token_error:      { type: 'error',   text: 'Failed to exchange Discord token. Please try again.' },
  user_fetch_error: { type: 'error',   text: 'Could not fetch your Discord profile. Please try again.' },
  no_user:          { type: 'error',   text: 'User session expired. Please log in again.' },
}
...
export default function Dashboard() {
  ...
  // Handle ?discord=... query param after OAuth callback
  useEffect(() => {
    const status = searchParams.get('discord')
    if (!status) return
    const msg = DISCORD_STATUS_MESSAGES[status]
    if (msg) setDiscordMessage(msg)
    if (status === 'linked' && refreshUser) refreshUser()
  ...
  const handleLinkDiscord = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/auth/discord/start`, {
      ...
      if (data.url) {
        ...
        setDiscordMessage({ type: 'error', text: 'Failed to start Discord link' })
        setDiscordLoading(false)
      }
    } catch (err) {
      setDiscordMessage({ type: 'error', text: 'Network error' })
      setDiscordLoading(false)
    }
  }

  const handleUnlinkDiscord = async () => {
    if (!confirm('Unlink your Discord account? You won\'t be able to redeem licenses until you link it again.')) return
    try {
      const token = localStorage.getItem('token')
      await fetch(`${API_URL}/api/auth/discord/unlink`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      })
      if (refreshUser) refreshUser()
      setDiscordMessage({ type: 'success', text: 'Discord unlinked.' })
    } catch (err) {
      setDiscordMessage({ type: 'error', text: 'Failed to unlink Discord' })
    }
  }

  const discordLinked = user?.discord?.linked
  ...
  return (
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {discordMessage && (
          <div className={`mb-6 px-4 py-3 rounded border text-sm font-mono ${
        ...
        {/* Discord link warning — only when not linked */}
        {!discordLinked && (
          <div className="mb-6 cyber-card rounded-lg p-5 bg-[#0a0a0f]/80 backdrop-blur-sm border-2 border-[#5865F2]/40">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded bg-[#5865F2]/10 border border-[#5865F2] flex items-center justify-center flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-white font-bold text-sm uppercase tracking-wider" style={{ fontFamily: 'Orbitron, sans-serif' }}>
                  Discord Link Required
                </div>
                <div className="text-xs text-[#888] mt-1">
                  You must link your Discord account before redeeming license keys or accessing products.
                </div>
              </div>
              <button
                onClick={handleLinkDiscord}
                disabled={discordLoading}
                className="px-5 py-2 bg-[#5865F2] text-white font-bold text-xs uppercase tracking-wider hover:bg-[#4752C4] transition-colors disabled:opacity-50 flex-shrink-0"
              >
